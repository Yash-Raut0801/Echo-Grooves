...
export async function updateCartIcon() {
  try {
    ...
    // Accept either { count: N } or { totalItems: N }
    const json = await res.json()
    const total = json.count ?? json.totalItems ?? 0

    document.getElementById('cart-banner').innerHTML =
      total > 0
        ? `<a href="/cart.html"><img src="/images/cart.png" alt="cart">${total}</a>`
        : ''
  } catch (err) {
...
function renderCartItems(items, cartList) {
  ...
  items.forEach((item) => {
    ...
    li.innerHTML = `
      ...
      <span>x ${item.quantity} = ₹${itemTotal.toFixed(2)}</span>
    `
